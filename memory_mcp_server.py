...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if m == "initialize":
            self._send(rid, {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}},
                "serverInfo": {"name": "vps-memory", "version": "2.0.0"}})
            return

        if m == "tools/list":
            self._send(rid, {"tools": [
                {"name": "add_memory", "description": "Save a memory for future recall",
                 "inputSchema": {"type": "object", "properties": {"text": {"type": "string"}, "category": {"type": "string"}}}},
                {"name": "search_memories", "description": "Search memories by keyword (FTS5) and/or semantic similarity",
                 "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "limit": {"type": "integer", "default": 5}}}},
                {"name": "get_recent", "description": "Get most recent memories",
                ...
                {"name": "memory_stats", "description": "Show memory count and embedding status"}
            ]})
            return
        ...
        if m == "tools/call":
            ...
            try:
                if nm == "add_memory":
                    ...
                    if not text:
                        self._error(rid, "No text provided"); return
                    cat = a.get("category", "general")
                    ts = datetime.datetime.now().isoformat()
                    mid = str(uuid.uuid4())[:8]
                    vecs = _embed([text])
                    emb = struct.pack("%df" % EMBED_DIM, *vecs[0]) if vecs else None
                    conn.execute("INSERT INTO memories (id, text, category, ts, embedding) VALUES (?,?,?,?,?)",
                                 (mid, text, cat, ts, emb))
                    conn.commit()
                    status = "with embedding" if emb else "FTS-only"
                    self._send(rid, {"content": [{"type": "text", "text": "SAVED [%s] (%s) [%s]: %s" % (mid, cat, status, text[:150])}]})

                elif nm == "search_memories":
                    q = a.get("query", "").strip()
                    if not q:
                        self._error(rid, "No query provided"); return
                    limit = a.get("limit", 5)
                    fts_results = {}
                    try:
                        rows = conn.execute(
                            ...
                            (q, limit * 3)).fetchall()
                        for r in rows:
                            fts_results[r[0]] = {"text": r[1], "category": r[2], "ts": r[3], "fts_rank": -r[4]}
                    except Exception:
                        rows = conn.execute(
                            "SELECT id, text, category, ts FROM memories WHERE text LIKE ? LIMIT ?",
                            ("%" + q + "%", limit * 3)).fetchall()
                        for r in rows:
                            fts_results[r[0]] = {"text": r[1], "category": r[2], "ts": r[3], "fts_rank": 1.0}

                    q_vecs = _embed([q])
                    vec_results = {}
                    if q_vecs:
                        q_vec = q_vecs[0]
                        all_rows = conn.execute("SELECT id, text, category, ts, embedding FROM memories WHERE embedding IS NOT NULL").fetchall()
                        scored = []
                        for r in all_rows:
                        ...
                        for sim, r in scored[:limit * 3]:
                            vec_results[r[0]] = {"text": r[1], "category": r[2], "ts": r[3], "vec_sim": sim}
                    ...
                    if vec_results:
                        ...
                        for mid in all_ids:
                            ...
                            merged.append((0.4 * fts_s + 0.6 * vec_s, fts_results.get(mid) or vec_results.get(mid)))
                        merged.sort(reverse=True)
                        lines = ["[%s] %s" % (r[1]["category"], r[1]["text"][:200]) for r in merged[:limit]]
                    else:
                        items = sorted(fts_results.values(), key=lambda x: x["fts_rank"], reverse=True)[:limit]
                        lines = ["[%s] %s" % (r["category"], r["text"][:200]) for r in items]

                    mode = "hybrid" if vec_results else "FTS-only"
                    self._send(rid, {"content": [{"type": "text", "text": "[%s] %s" % (mode, chr(10).join(lines) or "No matches")}]})

                elif nm == "get_recent":
                    n = a.get("limit", 10)
                    rows = conn.execute("SELECT category, text FROM memories ORDER BY ts DESC LIMIT ?", (n,)).fetchall()
                    lines = ["[%s] %s" % (r[0], r[1][:200]) for r in rows]
                    self._send(rid, {"content": [{"type": "text", "text": chr(10).join(lines) or "Empty"}]})

                elif nm == "get_all":
                    rows = conn.execute("SELECT category, text FROM memories WHERE id != 'init' ORDER BY ts DESC").fetchall()
                    lines = ["[%s] %s" % (r[0], r[1][:200]) for r in rows]
                    self._send(rid, {"content": [{"type": "text", "text": "%d memories:\n%s" % (len(lines), chr(10).join(lines))}]})

                elif nm == "delete_memory":
                    mid = a.get("id", "").strip()
                    if not mid:
                        self._error(rid, "No id provided"); return
                    cur = conn.execute("DELETE FROM memories WHERE id = ?", (mid,))
                    conn.commit()
                    if cur.rowcount:
                        self._send(rid, {"content": [{"type": "text", "text": "Deleted [%s]" % mid}]})
                    else:
                        self._error(rid, "Memory [%s] not found" % mid)

                elif nm == "memory_stats":
                    count = conn.execute("SELECT COUNT(*) FROM memories WHERE id != 'init'").fetchone()[0]
                    with_emb = conn.execute("SELECT COUNT(*) FROM memories WHERE embedding IS NOT NULL AND id != 'init'").fetchone()[0]
                    has_model = _load_embedder()
                    self._send(rid, {"content": [{"type": "text", "text": "Total: %d, with embeddings: %d, model loaded: %s" % (count, with_emb, has_model)}]})
